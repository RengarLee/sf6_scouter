# <img src="./images/logo.png" alt="SF6 Scouter Logo" width="40" valign="middle"> SF6 Scouter

**SF6 Scouter** 是一个面向《街头霸王6》玩家和主播的**全方位数据分析与智能对战助手**。除了无感的 **实时战绩追踪** 和 **Pro 悬浮面板**，更内置了 **深度数据分析**、**OCR 对手画像** 和 **防上头机制**，助你科学上分。

> **悬浮追踪分数，数据洞悉对手；手热连胜吃分，上头智能止损。**

[English](./README.md) | [简体中文] | [日本語](./README_ja.md)

[在爱发电支持项目](https://afdian.com/a/sf6scouter)

## 仓库说明

本仓库是 **SF6 Scouter 可运行应用的分发仓库**，用于提供网站、预览图、发布说明和下载入口。

- 这里提供的是可直接使用的应用及相关公开资源。
- 这里 **不包含应用源码**。


## 核心功能

- **无限对战记录保存**：官网仅保留最近 100 场对战记录，SF6 Scouter 持续积累你的每一场对战数据，历史记录永不丢失。
- **实时对战追踪**：记录当前会话中的胜负、胜率和分数变化。
- **深度数据与周期报表**：提供胜率矩阵、K线走势、各角色对阵胜率，以及每周战绩复盘。
- **VS画面OCR识别与对手画像**：在VS加载画面自动识别对手，并在应用内提示对手倾向，辅助判断。
- **防上头助手**：短时间连败时智能弹窗提醒，保护你的珍贵排位分。
- **LP / MR 双支持**：同时支持段位分和大师分，清晰展示增减幅度。
- **CFN 账号追踪**：登录一次后即可追踪自己的账号，也可以切换查看其他 CFN ID。
- **Mini & Pro 悬浮面板**：提供紧凑视图或图表展示丰富的专业视图。
- **OBS 透明支持**：方便叠加到直播画面中。
- **布局记忆**：自动记住窗口位置、大小、透明度和相关显示设置。
- **内置自动更新**：新版本可以直接在应用内获取。

## 功能预览

### 迷你模式

| LP 追踪 | MR 追踪 | 实时变化 | 透明模式 |
| :---: | :---: | :---: | :---: |
| ![LP 追踪](./images/lp.png) | ![MR 追踪](./images/mr.png) | ![实时变化](./images/change.gif) | ![透明模式](./images/Transparent.jpg) |

| 紧凑实时追踪 |
| :---: |
| ![Mini 演示](./images/mini.gif) |

### 专业面板

| 透明专业悬浮面板 | 适配 OBS 的直播布局 |
| :---: | :---: |
| ![Pro 面板](./images/Pro.gif) | ![OBS 布局](./images/ProWithOBS.gif) |

| 在 Pro 面板中查看数据分析 | 提示对手习惯 |
| :---: | :---: |
| ![Pro 数据分析](./images/v0_3_5_Pro_PlayData.gif) | ![对手习惯提示](./images/v0_3_5_user_image.jpg) |

### 数据分析页面

| 分数走势拆解 | 对战与角色洞察 | 周期报告复盘 |
| :---: | :---: | :---: |
| ![分数走势](./images/v0_3_3_K_Line.gif) | ![对战与角色洞察](./images/v0_3_5_PlayData.gif) | ![周期报告](./images/v0_3_7_Report.png) |

### 回放分析

通过回放 ID 查看整场复盘概览，结合回合时间轴、受伤事件与连段记录联动分析对局。

![回放分析演示](./images/v0_5_0_Replayer.gif)

## 使用方法

1. 下载并启动 `SF6 Scouter`。
2. 在弹出窗口中登录 **CFN**。
3. 默认追踪自己的账号，也可以输入其他 **CFN ID** 查看其他玩家。
4. 按需要在 **Mini** 和 **Pro** 之间切换。
5. 需要更深入的数据时，打开 **数据分析页面** 查看走势和明细。
6. 根据需求调整透明度、布局和显示方式，应用会自动记住你的设置。

## OBS 配置

1. 在 OBS 中添加 **窗口采集**。
2. 选择 `SF6 Scouter` 窗口。
3. 采集方式使用 **Windows 10 (1903 及以上)**。
4. 在应用内按需调整透明度和相关显示选项。

## 社区

- **Discord**: [加入 Discord](https://discord.gg/xg93c5mmx2)
- **QQ 群**: 扫描 `./images/qq.jpg` 中的二维码加入

| Discord | QQ 群 |
| :---: | :---: |
| ![Discord](./images/discord.png) | ![QQ](./images/qq.jpg) |

## 支持项目

如果 SF6 Scouter 对你的对战或直播有所帮助，欢迎通过 [爱发电](https://afdian.com/a/sf6scouter) 支持项目的持续开发与维护。感谢每一份支持！

## 安全性

发布版本会在分发前通过 [VirusTotal](https://www.virustotal.com/) 检查。

## 许可

本仓库采用 **GPL-3.0** 许可。

- **免责声明**：本项目与 Capcom 官方无关，使用风险由使用者自行承担。
- **分发说明**：本仓库分发的是可运行应用及相关资源，不是应用源码仓库。
